...
const rolesExistsValidate = async (id = 0) => {
    const response = await models.roles.findByPk(id);
    if (!response) {
        throw new Error('El rol ' + id + ' no esta registrado');
    }
}

const correoExistsValidate = async (email = '') => {

    const person = await models.persons.findOne({ where: { email } });
    if (person != null)
...
const administrativeExistsValidate = async (id) => {
    const exiteAdmin = await models.administratives.findByPk(id);
    if (!exiteAdmin) {
